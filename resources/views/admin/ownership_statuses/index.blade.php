...
@section('header', 'Ownership Status Lifecycle')

@section('content')
    <div class="flex justify-between items-center mb-10">
        <div>
            <h3 class="text-3xl font-black text-black tracking-tighter">Ownership Status</h3>
            <p class="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Manage asset ownership
                classifications</p>
        </div>
        <button onclick="openModal()"
            ...
            <i class='bx bx-plus-circle text-xl'></i> Add Classification
        </button>
    </div>
        ...
        <table id="mainTable" class="w-full text-sm text-left">
            <thead>
                <tr class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-50">
                    ...
                    <th class="px-6 py-5">Meta Details</th>
            ...
            <div
                ...
                <div class="px-8 pt-8 pb-8 bg-white sm:p-10">
                    <div class="flex justify-between items-center mb-8">
                        <h3 class="text-2xl font-black text-black tracking-tighter" id="modalTitle">Setup Configuration</h3>
                    ...
                    <form id="setupForm" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="md:col-span-2">
                                <label
                                    class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Display
                                    Title</label>
                                <input type="text" name="title" id="title"
                                    ...
                                    placeholder="e.g. Freehold" required>
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="description" id="description" rows="3"
                                    class="w-full rounded-2xl border-slate-100 bg-slate-50/50 text-sm p-4 focus:ring-4 focus:ring-blue-50 focus:border-[#02b8f2] transition-all"
                                    placeholder="Full details and disclosure guidelines..."></textarea>
                            </div>
                        </div>
        ...
        <script>
            $(document).ready(function () {
                var table = $('#mainTable').DataTable({
                    ajax: "{{ route('admin.ownership-statuses.index') }}",
                    responsive: true,
                    columns: [
                        ...
                        },
                        {
                            data: 'meta_title', render: function (d, t, r) {
                                return `<div class="text-[10px] font-bold text-slate-400 uppercase leading-tight">${d || 'No SEO Meta'}<br><span class="text-[#02b8f2]">Descr: ${r.meta_description ? 'Set' : 'Empty'}</span></div>`;
                            }
                        },
                        {
                ...
                $('#setupForm').submit(function (e) {
                    e.preventDefault();
                    var id = $('#recordId').val();
                    var url = id ? `/admin/ownership-statuses/${id}` : "{{ route('admin.ownership-statuses.store') }}";
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            closeModal();
                            table.ajax.reload();
                            Swal.fire({ icon: 'success', title: 'Asset State Updated', text: res.message, timer: 2000, showConfirmButton: false });
                        }
                    });
                });
            });
            ...
            function openModal() {
                $('#setupForm')[0].reset();
                $('#recordId').val('');
                $('#modalTitle').text('New Ownership Logic');
            ...
            function editRecord(id) {
                $.get(`/admin/ownership-statuses/${id}/edit`, function (d) {
                    $('#recordId').val(d.id);
                    $('#title').val(d.title);
                    ...
                    $('#modalTitle').text('Modify Ownership Logic');
                    $('#setupModal').removeClass('hidden');
                });
            }

            function deleteRecord(id) {
                Swal.fire({ title: 'Delete Configuration?', text: "This might affect existing property listings.", icon: 'warning', showCancelButton: true, confirmButtonColor: '#000', cancelButtonColor: '#f1f5f9' }).then((result) => {
                    if (result.isConfirmed) {
                        $.ajax({ url: `/admin/ownership-statuses/${id}`, type: 'DELETE', data: { _token: '{{ csrf_token() }}' }, success: () => { table.ajax.reload(); Swal.fire('Purged!', 'Registry updated.', 'success'); } });
                    }
                });
            }
        </script>
