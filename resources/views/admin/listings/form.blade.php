        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h3 class="text-3xl font-black text-slate-900 tracking-tight" id="pageTitle">
                    {{ isset($listing) ? 'Modify Listing' : 'New Property Listing' }}</h3>
                <p class="text-[11px] text-slate-400 font-bold uppercase tracking-[0.2em] mt-1">Portfolio Optimizer
                    <span class="mx-2 text-slate-200">•</span> Step <span id="currentStepText"
                        class="text-indigo-600">1</span> of 3</p>
            </div>
        </div>
    ...
    <form id="listingForm" enctype="multipart/form-data" class="flex flex-col flex-1">
            ...
            <div id="stage1" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div class="mt-2 flex items-center">
                        <input type="checkbox" id="has_discount" class="rounded border-slate-300 text-[#02b8f2] focus:ring-0 mr-2" onchange="toggleDiscountField()" {{ (isset($listing) && $listing->old_price) ? 'checked' : '' }}>
                        <label for="has_discount" class="text-xs font-bold text-slate-500 uppercase tracking-wider">Add Discount?</label>
                    </div>
                </div>

                <div id="discount_field_container" class="{{ (isset($listing) && $listing->old_price) ? '' : 'hidden' }}">
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Old Price (£)</label>
                    <div class="relative">
                        <input type="number" step="0.01" name="old_price" id="old_price" class="w-full rounded-lg border-slate-100 bg-slate-50/50 text-xs p-3 pl-8 focus:ring-blue-100 focus:border-[#02b8f2] transition-all" placeholder="0.00" value="{{ $listing->old_price ?? '' }}">
                        <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold">£</span>
                    </div>
                </div>
        ...
        <div class="px-8 py-6 bg-slate-50/50 border-t border-slate-100 flex justify-between rounded-b-3xl flex-shrink-0">
            ...
            <div class="flex-1 flex justify-end gap-3">
                <a href="{{ route('admin.listings.index') }}" class="px-6 py-3.5 text-slate-400 text-xs font-black uppercase tracking-widest hover:text-rose-500 transition-all flex items-center">Cancel</a>
                ...
                <button type="submit" id="submitBtn" class="hidden px-12 py-3.5 rounded-2xl bg-[#131B31] text-white text-xs font-black uppercase tracking-widest shadow-xl shadow-slate-900/10 active:scale-95 transition-all">Publish Property</button>
            </div>
        </div>
    </form>
</div>
