...
@section('content')
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
            <h3 class="text-slate-800 font-extrabold text-2xl tracking-tight">Drafts Pool</h3>
                ...
                <div class="px-8 pt-10 pb-4 flex-shrink-0">
                    <div class="flex justify-between items-center mb-8">
                        <div class="flex items-center gap-4">
                            <div
                                class="w-12 h-12 bg-blue-50 text-[#02b8f2] rounded-2xl flex items-center justify-center text-2xl rotate-3 shadow-inner">
                                <i class='bx bxs-home-circle'></i>
                            </div>
                            <div>
                                <h3 class="text-2xl font-black text-black tracking-tighter" id="modalTitle">New Property
                                    Listing</h3>
                                <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Global Portfolio •
                ...
                <form id="listingForm" enctype="multipart/form-data" class="flex flex-col flex-1 overflow-hidden">
                    ...
                    <div class="flex-1 overflow-y-auto px-8 py-2 pb-8">
                        ...
                        <div id="stage1" class="form-grid">
                            ...
                            <div class="full-width">
                                ...
                                <div class="relative">
                                    <input type="text" name="address" id="address"
                                        class="w-full rounded-lg border-slate-100 bg-slate-50/50 text-xs p-2 pl-11 focus:ring-blue-100 focus:border-[#02b8f2] transition-all"
                                        placeholder="Search UK address..." required>
                                    <i
                            ...
                            <div>
                                <label
                                    class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Price/Rent
                                    (£)</label>
                                <div class="relative">
                            ...
                            <div id="bedBathContainer">
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <select name="bedrooms" id="bedrooms" class="select2 w-full text-xs">
                                        @for($i = 0; $i <= 100; $i++)
                                        <option value="{{ $i }}">{{ $i }} Bed{{$i > 1 ? 's' : ''}}</option> @endfor
                                    </select>
                                    <select name="bathrooms" id="bathrooms" class="select2 w-full text-xs">
                                        @for($i = 0; $i <= 100; $i++)
                                        <option value="{{ $i }}">{{ $i }} Bath{{$i > 1 ? 's' : ''}}</option> @endfor
                                    </select>
                                </div>
                            </div>
                            <div class="full-width">
                                <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Full
                                    Description</label>
                                <textarea id="description_editor" class="w-full rounded-lg text-xs p-2" rows="6"></textarea>
                        ...
                        <div id="stage2" class="hidden">
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-4">Property
                                Features & Amenities</label>
                            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                                @foreach($features as $feature)
                                    <label
                                        class="group relative flex items-center p-2 bg-slate-50 rounded-lg border border-slate-100 hover:border-blue-100 hover:bg-white transition-all cursor-pointer">
                                        <input id="feat_{{ $feature->id }}" name="features[]" value="{{ $feature->id }}"
                                            type="checkbox"
                                            class="h-4 w-4 text-[#02b8f2] rounded border-slate-300 focus:ring-0">
                                        <span
                        ...
                        <div id="stage3" class="hidden space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="p-4 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                                    <label
                                        class="block text-xs font-bold text-slate-700 mb-3 flex items-center gap-2 italic">
                                    ...
                                    <label for="thumbnail"
                                        ...
                                        to Upload</label>
                                    <div id="thumbPreview" class="mt-3 hidden"><img src=""
                                            class="w-full h-32 object-cover rounded-lg shadow-lg"></div>
                                </div>
                                <div class="p-4 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                                    <label
                                        class="block text-xs font-bold text-slate-700 mb-3 flex items-center gap-2 italic">
                                    ...
                                    <label for="gallery"
                                        ...
                                        Multiple</label>
                                    <div id="galleryPreview" class="mt-3 flex flex-wrap gap-1.5"></div>
                                </div>
                            </div>
        ...
        <script>
            ...
            $(document).ready(function () {
                ...
                $('#listingForm').submit(function (e) {
                    ...
                    Swal.fire({ title: 'Publishing Listing...', didOpen: () => Swal.showLoading() });
                    $.ajax({
                        url: id ? `/admin/listings/${id}` : "{{ route('admin.listings.store') }}",
                        type: 'POST', data: formData, processData: false, contentType: false,
                        success: (res) => { closeModal(); table.ajax.reload(); Swal.fire('Published!', res.message, 'success'); },
                        error: (xhr) => {
            ...
            function initCKEditor() {
                ClassicEditor.create(document.querySelector('#description_editor'), {
                    ...
                    placeholder: 'Fully describe the property highlights and features...'
            ...
            function toggleUnitTypes(selectedUnitId = null) {
                ...
                unitSelect.empty().append('<option value="">Choose Category</option>');
                if (propTypeId) {
            ...
            // openModal function is now redefined above to include autocomplete initialization
