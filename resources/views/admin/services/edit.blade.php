        ...
        <form action="{{ route('admin.services.update', $service) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input
                    ...
                    id="title" type="text" name="title" value="{{ old('title', $service->title) }}" required>
                @error('title')
            ...
            <div class="mb-4">
                ...
                <select
                    ...
                    @foreach($users as $user)
                        <option value="{{ $user->name }}" {{ old('author', $service->author) == $user->name ? 'selected' : '' }}>{{ $user->name }}</option>
                    @endforeach
                </select>
                @error('author')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <textarea id="description" name="description" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('description') border-red-500 @enderror" rows="5">{{ old('description', $service->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                @if($service->image)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $service->image) }}" alt="{{ $service->title }}" class="h-20 w-20 object-cover rounded">
                    </div>
                @endif
                <input
