...
@section('header', 'Payment Frequency Setup')

@section('content')
    <div class="flex justify-between items-center mb-10">
        <div>
            <h3 class="text-3xl font-black text-black tracking-tighter">Rent Frequency</h3>
            <p class="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">Configure rental payment cycles</p>
        </div>
        <button onclick="openModal()"
            class="bg-black hover:bg-[#ff931e] text-white font-black py-4 px-8 rounded-2xl shadow-xl transition-all flex items-center gap-3 active:scale-95">
            <i class='bx bx-time-five text-xl'></i> Define Cycle
        </button>
    </div>
        ...
        <table id="mainTable" class="w-full text-sm text-left">
            <thead>
                <tr class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-50">
                    ...
                    <th class="px-6 py-5">Cycle Title</th>
                    <th class="px-6 py-5">SEO Status</th>
            ...
            <div
                ...
                <div class="px-8 pt-8 pb-8 bg-white sm:p-10">
                    <div class="flex justify-between items-center mb-8">
                        <h3 class="text-2xl font-black text-black tracking-tighter" id="modalTitle">Cycle Configuration</h3>
                    ...
                    <form id="setupForm" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="md:col-span-2">
                                <label
                                    class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Cycle
                                    Title</label>
                                <input type="text" name="title" id="title"
                                    class="w-full rounded-2xl border-slate-100 bg-slate-50/50 text-sm p-4 focus:ring-4 focus:ring-orange-50 focus:border-[#ff931e] transition-all font-bold"
                            ...
                            <div>
                                ...
                                <input type="text" name="meta_title" id="meta_title"
                                    class="w-full rounded-2xl border-slate-100 bg-slate-50/50 text-sm p-4 focus:ring-4 focus:ring-orange-50 focus:border-[#ff931e] transition-all"
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="description" id="description" rows="3"
                                    class="w-full rounded-2xl border-slate-100 bg-slate-50/50 text-sm p-4 focus:ring-4 focus:ring-orange-50 focus:border-[#ff931e] transition-all"
                                    placeholder="Instructions for brokers..."></textarea>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end pt-6 gap-4">
                            ...
                            <button type="submit"
                                class="px-10 py-4 bg-[#ff931e] text-white font-black text-[11px] uppercase tracking-widest rounded-2xl shadow-xl shadow-orange-200 active:scale-95 transition-all">Save
                                Cycle</button>
                        </div>
                    </form>
                </div>
        ...
        <script>
            $(document).ready(function () {
                var table = $('#mainTable').DataTable({
                    ajax: "{{ route('admin.rent-frequencies.index') }}",
                    responsive: true,
                    columns: [
                        ...
                        },
                        {
                            data: 'meta_title', render: function (d, t, r) {
                                return `<div class="text-[10px] font-bold text-slate-400 uppercase leading-tight">${d || 'No SEO Meta'}<br><span class="text-[#ff931e]">Status: Active</span></div>`;
                            }
                        },
                        {
                ...
                $('#setupForm').submit(function (e) {
                    e.preventDefault();
                    var id = $('#recordId').val();
                    var url = id ? `/admin/rent-frequencies/${id}` : "{{ route('admin.rent-frequencies.store') }}";
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            closeModal();
                            table.ajax.reload();
                            Swal.fire({ icon: 'success', title: 'Cycle Configured', text: res.message, timer: 2000, showConfirmButton: false });
                        }
                    });
                });
            });
            ...
            function openModal() {
                $('#setupForm')[0].reset();
                $('#recordId').val('');
                $('#modalTitle').text('New Rent Cycle');
            ...
            function editRecord(id) {
                $.get(`/admin/rent-frequencies/${id}/edit`, function (d) {
                    $('#recordId').val(d.id);
                    $('#title').val(d.title);
                    ...
                    $('#modalTitle').text('Modify Rent Cycle');
                    $('#setupModal').removeClass('hidden');
                });
            }

            function deleteRecord(id) {
                Swal.fire({ title: 'Delete Cycle?', text: "Ensure no active listings are using this frequency.", icon: 'warning', showCancelButton: true, confirmButtonColor: '#000', cancelButtonColor: '#f1f5f9' }).then((result) => {
                    if (result.isConfirmed) {
                        $.ajax({ url: `/admin/rent-frequencies/${id}`, type: 'DELETE', data: { _token: '{{ csrf_token() }}' }, success: () => { table.ajax.reload(); Swal.fire('Deleted!', 'Cycle removed.', 'success'); } });
                    }
                });
            }
        </script>
