...
// Tab functionality for search filters
document.querySelectorAll('.tab-btn').forEach(button => {
  button.addEventListener('click', () => {
    ...
    document.querySelectorAll('.tab-content1').forEach(content => {
...
// Advance form toggle
document.querySelectorAll('.show-form').forEach(btn => {
  btn.addEventListener('click', (e) => {
    e.preventDefault();
    const advanceForm = document.querySelector('.wd-search-form');
    if (advanceForm) {
      advanceForm.classList.toggle('active');
    }
  });
});
